...
#include "../../include/IHTTPTransaction.h"
...
bool    DOMParser::Parse                    ()
{
   ...
   HttpDownloader loHttp;
   std::string lstrRange;
   if(!loHttp.Init(this->url, lstrRange, MPD))
   	 return false;

   if(!loHttp.StartDownload())
   	return false;
   ...
   while(true)
   {
   	memset(lszRecv, 0, sizeof(lszRecv));

   	ret = loHttp.Read((uint8_t *)lszRecv, sizeof(lszRecv) -1);
   	if(ret < 0)
   		break;

   	if(ret > 0)
   		lstrContent.append(lszRecv);
   }

   this->reader = xmlReaderForMemory(lstrContent.c_str(), lstrContent.size(), NULL, NULL, 0);
