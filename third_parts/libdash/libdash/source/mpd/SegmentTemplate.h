    ...
    namespace mpd
    {
        class SegmentTemplate : public ISegmentTemplate, public MultipleSegmentBase
        {
            public:
                ...
                ISegment*           GetMediaSegmentFromNumber   (const std::vector<IBaseUrl *>& baseurls, const std::string& representationID, uint32_t bandwidth, uint32_t number,int segmentNum) const;
                ...
                ISegment*           GetMediaSegmentFromTime     (const std::vector<IBaseUrl *>& baseurls, const std::string& representationID, uint32_t bandwidth, uint64_t time,int segmentNum) const;
            ...
            private:
                ...
                void        FormatChunk64         (std::string& uri, uint64_t number) const;
                ISegment*   ToSegment           (const std::string& uri, const std::vector<IBaseUrl *>& baseurls, const std::string& representationID, uint32_t bandwidth, 
                                                 dash::metrics::HTTPTransactionType type, uint32_t number = 0, uint64_t time = 0,int segmentNum=-1) const;
