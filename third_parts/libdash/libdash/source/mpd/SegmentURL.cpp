...
ISegment*           SegmentURL::ToMediaSegment  (const std::vector<IBaseUrl *>& baseurls,int segmentNum/*=-1*/) const
{
    Segment *seg = new Segment();
    
    if(seg->Init(baseurls, this->mediaURI, this->mediaRange, dash::metrics::MediaSegment,segmentNum))
        return seg;
...
ISegment*           SegmentURL::ToIndexSegment  (const std::vector<IBaseUrl *>& baseurls,int segmentNum/*=-1*/) const
{
    Segment *seg = new Segment();
    
    if(seg->Init(baseurls, this->indexURI, this->indexRange, dash::metrics::IndexSegment,segmentNum))
        return seg;
