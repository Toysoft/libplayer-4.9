...
uint32_t    AbstractRepresentationStream::GetFirstSegmentNumber     ()
{
    if (this->mpd->GetType() == "dynamic")
    {
        ...
        uint32_t duration   = this->GetAverageSegmentDuration() / this->GetSegmentTimescale();
        uint32_t timeshift  = TimeResolver::GetDurationInSec(this->mpd->GetTimeShiftBufferDepth());

        // return 0 if no valid duration attibute, prevent crash here.
...
uint32_t    AbstractRepresentationStream::GetSegmentTimescale ()
{
    return 1;
}
