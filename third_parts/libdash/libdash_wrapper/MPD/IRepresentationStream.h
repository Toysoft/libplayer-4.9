        ...
        namespace mpd
        {
            ...
            class IRepresentationStream
            {
                public:
                    ...
                    virtual uint32_t                    GetSegmentTimescale       () = 0;

                    virtual uint64_t                    GetSegmentStartTimeByOffset(uint32_t offset) = 0;
                    // return value
                    // -2 : error; -1 : before ahead; >=0 : offset
                    virtual int32_t                     GetSegmentOffsetByTime(uint64_t timeToFind) = 0;
                    virtual uint32_t                   GetStartNumber() = 0;
                    virtual bool                         HasTimeLineOrNot() = 0;
            };
        }
    }
}
