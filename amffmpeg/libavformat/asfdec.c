...
typedef struct {
    ...
    char encrypt_sample_id[8];          ///for WMDRM-ND asf encryption
...
typedef int PRDecryptFunc(char* sample_id, uint8_t* data, uint32_t offset,
                int size, int type, int64_t pts, int obj_size);
typedef int PRSetDrmHeaderFunc(const void *headerData, size_t size, const void *licenseurl);
typedef int PRIsTVPFunc(void);
...
static PRIsTVPFunc* PRIsTVP = NULL;
...
static int get_PRWMV_func_handle()
{
    ...
    PRDecrypt = (PRDecryptFunc*)dlsym(gPRWMVLibHandle, "PRDecrypt");
    PRIsTVP = (PRIsTVPFunc*)dlsym(gPRWMVLibHandle, "PRisTVP");
    PRClose = (PRCloseFunc*)dlsym(gPRWMVLibHandle, "PRClose");

    if (PRSetDrmHeader == NULL || PRDecrypt == NULL || PRIsTVP == NULL || PRClose == NULL) {
...
static int drm_stronedrminfo(char *outpktdata, char *addr,
                      int size, unsigned int pts, int isdrminfo)
{
    drminfo_t drminfo;
...
/*add padding size*/
static int av_new_packet_tvp(AVPacket *pkt, int size)
{
    int ret = av_new_packet(pkt,size+sizeof(drminfo_t));
    pkt->size = size;
    return ret;
}
...
static void get_object_data(AVFormatContext *s, const char *key, int type, int len, int type2_size,char * data)
{
    char *value;
...
#define LEN 22
    ...
    if (*value) {
        av_dict_set(&s->metadata, key, value, 0);
                ...
                "get_tag_new [key:%s] in tag [%s].\n",  key,value);
                data=av_strdup(value);
    }
...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    unsigned int len;
    AVPacket pkt;
    char *protection_type=NULL;
    char *ASF_key_ID=NULL;
    char *ASF_license_url=NULL;
    char *ASF_extenc_content=NULL;
    unsigned int ASF_extenc_content_size=0;
    video_stream_num = 0;
        ...
        if (!ff_guidcmp(&g, &ff_asf_file_header)) {	    
            ...
            if (!s->keylen) {
                if (!ff_guidcmp(&g, &ff_asf_content_encryption)) {
                    ...
                    s->drmcontent = 1;
                    len= avio_rl32(pb);
                    av_log(s, AV_LOG_WARNING, "Secret data:\n");
                    av_get_packet(pb, &pkt, len);
                    av_hex_dump_log(s, AV_LOG_WARNING, pkt.data, pkt.size); av_free_packet(&pkt);
                    len= avio_rl32(pb);
                    get_object_data(s, "ASF_Protection_Type", -1, len, 32,protection_type);
                    len= avio_rl32(pb);
                    get_object_data(s, "ASF_Key_ID", -1, len, 32,ASF_key_ID);
                    len= avio_rl32(pb);
                    get_object_data(s, "ASF_License_URL", -1, len, 32,ASF_license_url);
                    ...
                    av_get_packet(pb, &pkt, len);
                    av_hex_dump_log(s, AV_LOG_WARNING, pkt.data, len);
                    ASF_extenc_content=pkt.data;
                    ASF_extenc_content_size=len;
...
//av_log(s, AV_LOG_INFO, "i=%d, st->codec->codec_type:%d, dar %d:%d sar=%d:%d\n", i, st->codec->codec_type, dar[i].num, dar[i].den, st->sample_aspect_ratio.num, st->sample_aspect_ratio.den);
    ...
    /*process DRM with PlayReady*/
    if (s->drmcontent && ASF_extenc_content && ASF_extenc_content_size) {
        if (video_stream_num == 0)//do not decrypt wma here
            return -1;
        if (!get_PRWMV_func_handle()) {
            int ret = PRSetDrmHeader(ASF_extenc_content,
                            ASF_extenc_content_size, ASF_license_url);
            if (!ret) {
                av_log(NULL, AV_LOG_ERROR, "PlayReady header format error!!!%d\n", ret);
            } else {
                s->drmcontent=0;
                s->iformat->name = "asf-pr";
                s->flags |= AVFMT_FLAG_PR_WMV;
                av_log(NULL, AV_LOG_WARNING, "PlayReady DRM protected stream detected,"
                            "but we can decoding it!\n");
                if (PRIsTVP()) {
                    s->flags |= AVFMT_FLAG_PR_TVP;
                    s->pb->is_slowmedia = 0;
                    av_log(NULL, AV_LOG_ERROR, "PlayReady TVP\n");
                } else
                    av_log(NULL, AV_LOG_ERROR, "PlayReady NOTVP\n");
            }
        }
    }
    return 0;
...
//            av_log(s, AV_LOG_DEBUG, "\n");
        ...
        }else if(asf->packet_replic_size >= 18){
            avio_skip(pb,asf->packet_replic_size-16); //sample id -> last 8 bytes typically
            avio_read(s->pb, &asf->encrypt_sample_id, 8);
            av_log(s, AV_LOG_ERROR,"get sample id for WMDRM-ND\n");
            av_hex_dump_log(s, AV_LOG_ERROR, asf->encrypt_sample_id, 8);
        }else
...
static int ff_asf_parse_packet(AVFormatContext *s, AVIOContext *pb, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        if (   asf_st->pkt.size != asf->packet_obj_size
            ...
            /* new packet */
            if (s->flags & AVFMT_FLAG_PR_TVP)
                av_new_packet_tvp(&asf_st->pkt, asf->packet_obj_size);
            else
...
//s->streams[asf->stream_index]->codec->codec_type == AVMEDIA_TYPE_AUDIO, asf->packet_obj_size);
        ...
        if (s->flags & AVFMT_FLAG_PR_TVP)
            ret = avio_read(pb,
                      asf_st->pkt.data + sizeof(drminfo_t) + asf->packet_frag_offset,
                      asf->packet_frag_size);
        else
        ...
        if (ret != asf->packet_frag_size) {
            ...
            } else if (s->flags & AVFMT_FLAG_PR_WMV) {//scrambling
                av_log(s, AV_LOG_ERROR, "prwmv reread packet from %lld, size %d\n",
                                asf_st->packet_pos, asf_st->pkt.size);
                avio_seek(pb, asf->packet_pos, SEEK_SET);
                asf_reset_header(s);
                ff_asf_get_packet(s, s->pb);
                asf->packet_time_start = 0;
                continue;
            } else
        ...
        if (s->key && s->keylen == 20)
            ff_asfcrypt_dec(s->key, asf_st->pkt.data + asf->packet_frag_offset,
                            ret);
        if (s->flags & AVFMT_FLAG_PR_WMV && PRDecrypt) {
            if (s->flags & AVFMT_FLAG_PR_TVP)
                PRDecrypt(asf->encrypt_sample_id,
                            asf_st->pkt.data + sizeof(drminfo_t) + asf->packet_frag_offset,
...
{
    asf_reset_header(s);
    if ((s->flags & AVFMT_FLAG_PR_WMV) && PRClose) {
        PRClose();
    }
    return 0;
}
