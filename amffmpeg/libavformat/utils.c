...
#define HAVE_INFO_NET_MAX_WAIT_PROBE_TIME_US (30*1000*1000) /**/
    ...
    if (fast_switch)
    {
        if (!strcmp(ic->iformat->name, "flv") && ic->pb && (0 == ic->pb->local_playback))
        {
        ...
        else if (ic->pb && ic->pb->local_playback)
        {
            ...
            ic->probesize = 10*1024*1024; /*enlarge probe size for local playing*/
        }
        else
        {
