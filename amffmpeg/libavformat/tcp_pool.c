...
int tcppool_opened_tcplink(URLContext *h, const char *uri, int flags)
{
    ...
    item->item_data = (unsigned long)h;
    link->h = h;
    link->uri = strdup(uri);
    link->flags = flags;
...
{
    struct item *item = itemlist_get_match_item(&tcppool_list, (unsigned long)h);
    struct tcppool_link *link;
    if (!item)
        return -1;
    link=(struct tcppool_link *)&item->extdata[0];
...
int tcppool_close_tcplink(URLContext *h)
{
    struct item *item = itemlist_get_match_item(&tcppool_list, (unsigned long)h);
    if (!item) {
        struct item *item = itemlist_get_match_item(&tcppool_list_free, (unsigned long)h);
        if (!item) {
            return -1;
        }
    }
