...
static int matrosk_ffio_limit(AVFormatContext *ctx, AVIOContext *s, int size)
{
    if(ctx->file_size>=0){
        int64_t remaining= ctx->file_size- avio_tell(s);
        if(remaining < size){
            int64_t newsize= avio_size(s);
            if(!ctx->file_size ||ctx->file_size<newsize)
                ctx->file_size= newsize - !newsize;
            remaining= ctx->file_size - avio_tell(s);
            remaining= FFMAX(remaining, 0);
        }

        if(ctx->file_size>=0 && remaining+1 < size){
                           ...
                           EbmlSyntax *syntax, void *data)
{
    ...
    AVFormatContext *ctx = matroska->ctx;
    AVIOContext *pb = matroska->ctx->pb;
    ...
    default:
        if(matrosk_ffio_limit(ctx, pb, length) != length)
            return AVERROR(EIO);
    ...
    for (i=0; i < matroska->tracks.nb_elem; i++) {
        ...
        if (!strcmp(track->codec_id, "V_MS/VFW/FOURCC")
            ...
            if (strstr(track->codec_id, "SBR")) {
                ...
                extradata_size = 2;
        } else if (codec_id == CODEC_ID_ALAC && track->codec_priv.size && track->codec_priv.size < INT_MAX - 12 - FF_INPUT_BUFFER_PADDING_SIZE) {
        ...
        if (track->type == MATROSKA_TRACK_TYPE_VIDEO) {
            ...
            if (st->codec->codec_id == CODEC_ID_SSA ||
                st->codec->codec_id == CODEC_ID_ASS)
#else
            if (st->codec->codec_id == CODEC_ID_ASS)
#endif
                                ...
                                uint8_t *additional, uint64_t additional_id, int additional_size)
{
    ...
    if (st->codec->codec_id == CODEC_ID_WAVPACK) {
        uint8_t *wv_data;
                              ...
                              int64_t timestamp, int flags)
{
    ...
    if ((index = av_index_search_timestamp(st, timestamp, flags)) < 0) {
        if (st->codec->codec_id == CODEC_ID_VP9)
            timestamp = 0; /*only one Key frame in vp9. playback to start*/
