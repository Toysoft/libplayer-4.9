...
static void *circular_buffer_task( void *_URLContext)
{
    ...
    while (1) {
        ...
        len = recv(s->udp_fd, s->tmp, sizeof(s->tmp), 0);
        pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &old_cancelstate);
        pthread_mutex_lock(&s->mutex);
        if (len < 0) {
...
static int udp_open(URLContext *h, const char *uri, int flags)
{
    ...
    //av_log(h, AV_LOG_ERROR, "[%s %d] is_output:%d\n", __FUNCTION__, __LINE__, is_output);
...
//	av_log(h, AV_LOG_ERROR, "[%s %d] rw_timeout:%lld", __FUNCTION__, __LINE__, h->rw_timeout);
    ...
    //av_log(h, AV_LOG_ERROR, "[%s %d] is_multicast:%d local_port:%d\n", __FUNCTION__, __LINE__, s->is_multicast, s->local_port);

    if ((s->is_multicast || s->local_port <= 0) && (h->flags & AVIO_FLAG_READ))
        s->local_port = port;


    //av_log(h, AV_LOG_ERROR, "[%s %d] localaddr[0]:%d\n", __FUNCTION__, __LINE__, localaddr[0]);
    ...
    //av_log(h, AV_LOG_ERROR, "[%s %d] reuse_socket:%d is_broadcast:%d\n", __FUNCTION__, __LINE__, s->reuse_socket, s->is_broadcast);
    ...
    //av_log(h, AV_LOG_ERROR, "[%s %d] local_port:%d is_multicast:%d\n", __FUNCTION__, __LINE__, s->local_port, s->is_multicast);
    ...
    if (s->is_multicast) {
        ...
        if (h->flags & AVIO_FLAG_READ) {
            /* input */
            //av_log(h, AV_LOG_ERROR, "[%s %d] num_include_sources:%x num_exclude_sources:%x\n", __FUNCTION__, __LINE__, num_include_sources, num_exclude_sources);
    ...
    //av_log(h, AV_LOG_ERROR, "[%s %d] is_connected:%d udp_fd:%d\n", __FUNCTION__, __LINE__, s->is_connected, udp_fd);
    ...
    //av_log(h, AV_LOG_ERROR, "[%s %d] is_output:%d circular_buffer_size:%d\n", __FUNCTION__, __LINE__, is_output, s->circular_buffer_size);
    ...
    if (s->fifo) {
        ...
        do {
            ...
            if (avail) { // >=size) {
                ...
                pthread_mutex_unlock(&s->mutex);
//                av_log(NULL, AV_LOG_ERROR, "[%s %d] %d\n", __FUNCTION__, __LINE__, avail);
                return size;
